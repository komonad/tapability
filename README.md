...
so the whole board and the status bar are always visible.

```
cargo run --release                 # play
cargo run --release -- --print 1    # print a puzzle + its solution
cargo run --release -- --bench 20   # time 20 generations
cargo test --release                # 29 tests, incl. uniqueness + minimality
```
...
| Esc | close the settings, otherwise quit |
...
* **spotlight** 鈥?adding a wall highlights the whole connected wall group it
  joined (blue, gold outline). Shift-hovering does the same for any group.
* **red clue** 鈥?if the walls and empty marks around a clue can no longer form
  its numbers, the clue turns red.
* **red walls** 鈥?a wall group that can never join the main wall group any more,
  ...
  alone.
* **red 2x2** 鈥?any 2x2 block of walls is outlined in red.
...
(R), Check (C), Solution (S) and Undo (Z) - and the generation settings are
editable in place. There is no menu to open: everything is on screen.
...
budget never breaks uniqueness 鈥?it only leaves a redundant clue in place.

Measured on this machine: 12x12 generates in ~15 ms, 20x20 in ~1.0 s average,
30x30 in 5鈥?0 s (it needs a larger `node_budget` to stay minimal).

## Code map

| file | contents |
|---|---|
| `src/model.rs` | grid geometry, cell states, clue encoding (`mask -> run lengths`), rule validation, live rule analysis |
...
| `src/config.rs` | generation settings (file, command line, in-game panel) |
...
1. **clue cells are empty** 鈥?forced;
2. **arc consistency per clue** 鈥?every clue has a precomputed set of legal
   ...
   branch is dead;
3. **no 2x2 black** 鈥?three black cells force the fourth white. Only the four
   windows around a cell that just turned black are examined;
4. **connectivity** 鈥?the cells that could still become black are split into
...
1. **grow a random solution whose black cells form a tree.** Each new black cell
   ...
   density is 40鈥?0%, which keeps the leftover white patches small.
...
random clues until unique needs **117鈥?64** clues, while removing from the
maximal set down to minimal needs **69鈥?3**. The adding direction is also slower
here, because its intermediate puzzles are nearly unique 鈥?exactly where the
solver has to search hard 鈥?whereas the removing direction mostly works on dense
...
| metric | result |
|---|---|
| generation time | 120鈥?500 ms, ~1000 ms average, 0 failures |
| search nodes per uniqueness proof | 35鈥?2000, adaptive pool |
| clues per puzzle | 69鈥?3 (inclusion-minimal) |
| black cells | 165鈥?99 of 400 (41鈥?0%) |
| biggest clue-free patch | 8鈥?9 cells (2鈥?0% of the board) |
| clue-free rows / columns | 0鈥? of 20 |
| test suite | 29 tests, ~1.1 s release |
